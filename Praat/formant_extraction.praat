form arguments
	...
	comment Where are the TextGrids and .wav files located?
		text inDir C:\Users\Tiamat\Dropbox\GIT\Raw_audio_pipeline\Raw-audio-pipeline\samples\mfa_aligned\

	comment What tier are the target segments on?
		integer seg_tier 2

	comment How many formant samples should be taken from each interval?
		integer form_slice_count 9
...
outputname_form$ =  "'inDir$'\formant_tracks.txt"
...
for k from 1 to strnum
	...
	# with an 2-symbol code that corresponds to the
	# speaker code.
	select Strings fileList
	fstr$ = Get string... 'k'
	spkcodeTMP$ = left$(fstr$, 2)
...
for snum from 1 to spkrnum
	...
	for k from 1 to spkrwavnum
		...
		for i from 1 to 'numint'
			...
			if ((startsWith(interval$,"A") or startsWith(interval$,"E") or startsWith(interval$,"I") or startsWith(interval$,"O") or startsWith(interval$,"U")))
